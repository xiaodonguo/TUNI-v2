...
class eeemodelLoss(nn.Module):
    ...
    def CCM_Loss(self, sideout, targets):
        ...
        labels_onehot = torch.zeros(n, 12, h, w)
        labels_onehot = labels_onehot.cuda()
        labels_onehot.scatter_(1, semantic_gt.view(n, 1, h, w), 1)
        labels_onehot = labels_onehot[:, 2:, :, :]  # Ignore unlabeled class-0, focusing on known classes.
        # print(labels_onehot.shape)

        kl_distance = nn.KLDivLoss(reduction='none')
        aux_pred_rgb_fore = aux_pred_rgb[:, 2:, :, :]  # Ignore unlabeled class-0, focusing on foreground classes.
        discrepancy_rgb = torch.sum(kl_distance(self.log_sm(aux_pred_rgb_fore), self.sm(labels_onehot)), dim=1)
        # normalize to 0-kd_loss
        Diff_rgb = min_max_norm(discrepancy_rgb.unsqueeze(1))
        aux_pred_t_fore = aux_pred_t[:, 2:, :, :]  # Ignore unlabeled class-0, focusing on foreground classes.
