...
from Loss.loss_helper import FSAuxCELoss, FSAuxRMILoss, FSCELoss
...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y):  # x: feature, y_hat: GT, y: pre
        batch_size, feat_dim = X.shape[0], X.shape[-1]  # batch=8
        ...
        for ii in range(batch_size):
            ...
            this_classes = [x for x in this_classes if (this_y == x).nonzero().shape[0] > self.max_views]   # self.max_views=100
        ...
        n_view = min(n_view, self.max_views)  # n_view=采样数量
        ...
        for ii in range(batch_size):
            ...
            for cls_id in this_classes:
                hard_indices = ((this_y_hat == cls_id) & (this_y != cls_id)).nonzero()  # 预测错误的类别list[]
                easy_indices = ((this_y_hat == cls_id) & (this_y == cls_id)).nonzero()  # 预测正确的类别list[]

                num_hard = hard_indices.shape[0] # 预测错误的类别的总数
                num_easy = easy_indices.shape[0] # 预测正确的类别的总数

                if num_hard >= n_view / 2 and num_easy >= n_view / 2:  # hard sample
                ...
                perm = torch.randperm(num_hard)  # 生成一个长度为num_hard随机排列的整数张量
    ...
    def _contrastive(self, feats_, labels_):
        ...
        neg_logits = neg_logits.sum(1, keepdim=True)  # negtive sample
    ...
    def forward(self, feats, labels=None, predict=None):
        ...
        predict = torch.nn.functional.interpolate(predict,
                                                 (feats.shape[2], feats.shape[3]), mode='nearest')  # downsample label and predict to feature's size
...
class ContrastCELoss(nn.Module, ABC):
    ...
    def forward(self, preds, features, target):
...
class ContrastAuxCELoss(nn.Module, ABC):
    ...
    def forward(self, preds, target, with_embed=False):
        ...
        loss = self.seg_criterion([pred_aux, pred], target)  # CE Loss
