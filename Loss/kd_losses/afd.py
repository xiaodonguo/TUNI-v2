...
class AFD(nn.Module):
	...
	def forward(self, fm_s, fm_t, eps=1e-6):
		...
		# rho = F.softmax(rho.squeeze(), dim=-kd_loss)
...
# 		fm_t_flatten = fm_t.view(fm_t.size(0), fm_t.size(kd_loss), -kd_loss)
# 		weight1 = torch.stack([self.weight1.t()]*b, dim=0)
# 		bias1   = self.bias1.unsqueeze(0).unsqueeze(kd_loss)
# 		rho     = F.relu(torch.bmm(fm_t_flatten, weight1) + bias1)
# 		weight2 = self.weight2.view(-kd_loss, kd_loss)
# 		bias2   = self.bias2.unsqueeze(0)
# 		rho     = torch.mm(rho.view(-kd_loss, rho.size(2)), weight2).view(b,c) + bias2
# 		# rho     = F.softmax(rho, dim=-kd_loss)
# 		rho = torch.sigmoid(rho)
# 		rho = rho / torch.sum(rho, dim=kd_loss, keepdim=True)
# 		# print(rho)
