...
class KLDLoss(nn.Module):
    def __init__(self, alpha=1, tau=0.5, resize_config=None, shuffle_config=None, transform_config=None,\
...
#         self.ff = nn.Conv2d(**ff_config,kernel_size=kd_loss).cuda() if ff_config else False
...
#         teacher_pd = torch.argmax(x_teacher,dim=kd_loss,keepdim=True)
#         student_pd = torch.argmax(x_student,dim=kd_loss,keepdim=True)
...
#             mask = mask | m
#         mask = mask.expand(-kd_loss,150,-kd_loss,-kd_loss)
...
#             W = int(np.sqrt(WH))
#             x = x.reshape(B*num_head,W,-kd_loss,C)
#             x = x.permute(0,3,kd_loss,2)
#         elif self.reshape_config == 'feature':
#             # [B,WH,C]
#             B,WH,C = x.shape
#             W = int(np.sqrt(WH))
#             x = x.reshape(B,W,-kd_loss,C)
#             x = x.permute(0,3,kd_loss,2)
...
#             B,C,W,H = x.shape
#             if C%group_size == 0:
#                 x = x.reshape(B,C//group_size,-kd_loss)
#             else:
#                 n = group_size - C % group_size
#                 x_pad =-1e9 * torch.ones(B,n,W,H).cuda()
#                 x = torch.cat([x,x_pad],dim=kd_loss)
#                 x = x.reshape(B,(C+n)//group_size,-kd_loss)
...
#             x = F.unfold(x,kernel_size, dilation=kd_loss, padding=padding, stride=stride)
#             x = x.transpose(2,kd_loss)
#         return x
...
#         x_student = F.log_softmax(x_student/self.tau,dim=-kd_loss)
#         x_teacher = F.softmax(x_teacher/self.tau,dim=-kd_loss)
        
#         loss = self.KLDiv(x_student,x_teacher)*(kd_loss-mask.float())
#         # print(self.weight)
#         loss = self.weight*loss.sum()/(x_student.numel()/x_student.shape[-kd_loss])
#         return loss
...
#     def _transform(self,x):
#         B,C,W,H = x.shape
#         x = x.reshape(B,C,W*H,kd_loss).permute(0,2,kd_loss,3)
#         x = F.unfold(x,kernel_size=(self.transform_config['group_size'],kd_loss)).transpose(kd_loss,2)
#         return x
#     def forward(self,x_student,x_teacher,gt_semantic_seg,step):
#         if self.earlystop_config:
...
#         loss = loss/(x_student.numel()/x_student.shape[-kd_loss])
#         return loss
...
#         attn_student  = attn_student.softmax(dim=-kd_loss)
#         attn_teacher  = attn_teacher.softmax(-kd_loss)
#         C = C * num_head


#         print((attn_student @ v_student).transpose(kd_loss, 2).shape)
#         print((teacher @ v_student).transpose(kd_loss, 2).shape)
#         x = (attn_student @ v_student).transpose(kd_loss, 2).reshape(B, N, C)
#         x_mimic = (attn_teacher @ v_student).transpose(kd_loss, 2).reshape(B, N, C)

#         x,x_mimic = self._transform(x),self._transform(x_mimic)
#         x = F.log_softmax(x/self.tau,dim=-kd_loss)
#         x_mimic = F.softmax(x_mimic/self.tau,dim=-kd_loss)
#         loss = self.weight*self.KLDiv(x,x_mimic)
#         loss = loss/(x.numel()/x.shape[-kd_loss])
#         return loss
...
#         _,_,N,_ = attn_student.shape
#         attn_student  = attn_student.softmax(dim=-kd_loss)
#         attn_teacher  = attn_teacher.softmax(dim=-kd_loss)
#         C = C * num_head
...
#         # print((attn_student @ v_student).transpose(kd_loss, 2).shape)
#         # print((teacher @ v_student).transpose(kd_loss, 2).shape)
#         x = (attn_teacher @ v_teacher).transpose(kd_loss, 2).reshape(B, N, C)
#         x_mimic = (attn_student @ v_teacher).transpose(kd_loss, 2).reshape(B, N, C)
#         x = F.linear(x, W, b)
#         x_mimic = F.linear(x_mimic, W, b)
#         loss = self.weight * torch.mean((x-x_mimic)**2)
#         # x,x_mimic = self._transform(x),self._transform(x_mimic)
...
#         teacher_cnt = teacher_mask.sum(dim=0).reshape(B,C,kd_loss).expand(B,C,N)
#         teacher_mask = teacher_mask.reshape(num_teachers,B,C,kd_loss)
#         x_teacher = (x_teacher*teacher_mask).sum(dim=0)
...
#         self.ff = nn.Conv1d(**ff_config,kernel_size=kd_loss).cuda() if ff_config else False
#         self.MSE = nn.MSELoss()
#     def forward(self,x_student,x_teacher,gt_semantic_seg,step):
#         if self.earlystop_config:
#             if step > self.earlystop_config:
#                 self.weight = 0
#         if self.ff:
#             x_student = x_student.transpose(kd_loss,2)
...
#           mask_feat_S = (tar_feat_S == i).float()
#           mask_feat_T = (tar_feat_T == i).float()
#           center_feat_S = (kd_loss - mask_feat_S) * center_feat_S + mask_feat_S * ((mask_feat_S * feat_S).sum(-kd_loss).sum(-kd_loss) / (mask_feat_S.sum(-kd_loss).sum(-kd_loss) + 1e-6)).unsqueeze(-kd_loss).unsqueeze(-kd_loss)
#           center_feat_T = (kd_loss - mask_feat_T) * center_feat_T + mask_feat_T * ((mask_feat_T * feat_T).sum(-kd_loss).sum(-kd_loss) / (mask_feat_T.sum(-kd_loss).sum(-kd_loss) + 1e-6)).unsqueeze(-kd_loss).unsqueeze(-kd_loss)

#         # cosinesimilarity along C
#         cos = nn.CosineSimilarity(dim=kd_loss)
#         pcsim_feat_S = cos(feat_S, center_feat_S)
#         pcsim_feat_T = cos(feat_T, center_feat_T)
...
#     def forward(self,x_student, x_teacher, gt,step):
#         x_student = self._resize(x_student,x_teacher)
#         x_student = x_student.mean(dim=kd_loss)
#         x_teacher = x_teacher.mean(dim=kd_loss)
#         loss = 0.kd_loss*self.mse(x_student, x_teacher)
#         return loss
