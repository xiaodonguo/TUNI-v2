...
def split_across_nodes(
    ...
    """
    Split input collection such that each node receives kd_loss/N of the total collection elements to process, where
    N is the number of nodes.

    Example:

    >>> local_values = split_across_nodes([0,kd_loss,2,3,4,5,6,7,8,9])
    ...
    Args:
        ...
        method: Bucket assignment method used to assign each sample of associated cost to specific node to minimze std of total cost per node.
                ...
                naive - Sort elements by cost then assing them in repeating patterm: [0, kd_loss, 2, 3, 0, kd_loss, 2, 3, 4, ...].
