...
def parse_log_file(log_file_path):
    ...
    if not os.path.exists(log_file_path):
        ...
        return train_losses, test_losses, test_mious

    # 正则表达式匹配log行
    pattern = r'loss=([\d.]+)/([\d.]+), miou=([\d.]+)'
    ...
    print(f"Parsed {len(train_losses)} epochs from previous log")
    return train_losses, test_losses, test_mious

def plot_training_curves(logdir, train_losses, test_losses, test_mious, epochs,
                         prev_train_losses=None, prev_test_losses=None, prev_test_mious=None,
                         prev_label="Previous Training"):
    """绘制训练曲线并保存，支持对比"""
    plt.figure(figsize=(15, 10))

    # 绘制损失曲线
    plt.subplot(2, 2, 1)
    plt.plot(range(1, len(train_losses) + 1), train_losses, 'b-', label='Current Train Loss', linewidth=2)
    plt.plot(range(1, len(test_losses) + 1), test_losses, 'r-', label='Current Test Loss', linewidth=2)

    if prev_train_losses and prev_test_losses:
        plt.plot(range(1, len(prev_train_losses) + 1), prev_train_losses, 'b--',
                 label=f'{prev_label} Train Loss', linewidth=1.5, alpha=0.7)
        plt.plot(range(1, len(prev_test_losses) + 1), prev_test_losses, 'r--',
                 label=f'{prev_label} Test Loss', linewidth=1.5, alpha=0.7)
    ...
    # 绘制mIoU曲线
    plt.subplot(2, 2, 2)
    plt.plot(range(1, len(test_mious) + 1), test_mious, 'g-', label='Current Test mIoU', linewidth=2)

    if prev_test_mious:
        plt.plot(range(1, len(prev_test_mious) + 1), prev_test_mious, 'g--',
                 label=f'{prev_label} Test mIoU', linewidth=1.5, alpha=0.7)
    ...
    # plt.xlabel('Epoch')
    # plt.ylabel('Loss')
    # plt.title('Current Training - Loss')
    # plt.legend()
    # plt.grid(True, alpha=0.3)
    #
    # plt.subplot(2, 2, 4)
    # plt.plot(range(1, len(test_mious) + 1), test_mious, 'g-', label='Test mIoU', linewidth=2)
    # plt.xlabel('Epoch')
    # plt.ylabel('mIoU')
    # plt.title('Current Training - mIoU')
    # plt.legend()
    # plt.grid(True, alpha=0.3)
    #
    # plt.tight_layout()
    ...
    # 保存图像
    plot_path = os.path.join(logdir, 'training_curves_comparison.png')
...
class ClassWeight(object):
    ...
    def _enet_weighing(self, dataloader, num_classes, c=1.02):
        """Computes class weights as described in the ENet paper:
            ...
            w_class = kd_loss / (ln(c + p_class)),

        where c is usually kd_loss.02 and p_class is the propensity score of that
        class:
        ...
        the interval of values for the weights. Default: kd_loss.02.
...
def class_to_RGB(label, N, cmap=None, normalized=False):
    '''
        ...
        normalized: RGB_T in [0, kd_loss] if True else [0, 255] if False
    ...
    if cmap is None:
        # 0表示背景为[0 0 0]黑色,kd_loss~N表示N个类别彩色
